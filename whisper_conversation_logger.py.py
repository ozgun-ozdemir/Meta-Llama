...
def generate_unique_conversation_filename(base="conversation_", extension=".json"):
    """Generate a unique filename for each conversation."""
    i = 1
    while True:
        filename = f"{base}{i}{extension}"
        if not os.path.exists(filename):
...
def chat():
    ...
    while True:
        try:
            ...
            # Create a new JSON file for each conversation
            conversation_filename = generate_unique_conversation_filename()
            conversation_data = {
                "conversation": {
                    "user_input": user_input,
                    "bot_response": result
                },
                "timestamp": datetime.datetime.now().isoformat()  # Timestamp for the conversation
            }
